// Registry and pack metadata validation.
const fs = require('fs');
...
const jsonFiles = [
  path.join(bpRoot, 'manifest.json'), path.join(rpRoot, 'manifest.json'),
  path.join(bpRoot, 'items', 'hero_focus.json'), path.join(rpRoot, 'entity', 'hero_base.entity.json'),
  path.join(rpRoot, 'render_controllers', 'hero_base.render.json'), path.join(rpRoot, 'models', 'entity', 'superhero_base.geo.json'),
  path.join(rpRoot, 'animation_controllers', 'hero_base.controller.json'), path.join(rpRoot, 'particles', 'energy_burst.particle.json'),
  path.join(rpRoot, 'texts', 'en_US.lang'), path.join(rpRoot, 'texts', 'tr_TR.lang')
];
for (const file of jsonFiles) {
  try { JSON.parse(file.endsWith('.lang') ? '{}' : fs.readFileSync(file, 'utf8')); }
  catch (error) { throw new Error(`JSON parse failed for ${file}: ${error.message}`); }
}
